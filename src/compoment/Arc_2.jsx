...
const Arc_2 = ({ value = 50, max = 150, size = 200, thickness = 7, color = '#08ff00' }) => {
    ...
    return (
        <Box
            ...
            top='18%' // ปรับตำแหน่งจากด้านบน
            left='63.5%' // ปรับตำแหน่งจากด้านซ้าย
            transform='translateX(-50%) rotate(48deg)' // เอียงกล่องที่ -10 องศา
        ...
        >
            <svg width={size} height={size} style={{ position: 'relative', display: 'inline-block' }}>
                ...
                <text
                    ...
                    transform={`rotate(-47 ${size / 2} ${size / 2})`} // หมุนข้อความที่จุดกลาง
                    style={{ 
                        pointerEvents: 'none', 
                        zIndex: 3, 
                        filter: 'drop-shadow(1px 1px 1px rgba(0, 0, 0, 0.7))', // เงาสีดำที่เบลอ
