...
const Arc_3 = ({ value = 50, max = 150, size = 200, thickness = 5, color = '#08ff00' }) => {
    ...
    return (
        <Box
            bg='none'
            width='15%' // ขนาดของกล่อง
            height='30%' // ขนาดของกล่อง
            position='fixed' // ใช้ fixed position
            top='66%' // ปรับตำแหน่งจากด้านบน
            left='65.5%' // ปรับตำแหน่งจากด้านซ้าย
            transform='translateX(-50%) rotate(121deg)' // เอียงกล่องที่ -10 องศา
        ...
        >
            <svg width={size} height={size} style={{ position: 'relative', display: 'inline-block' }}>
                ...
                <text
                    ...
                    textAnchor="middle"
                    dy=".3em"
                    fontSize={`${size * 1.5}%`} // ขนาดของฟอนต์เป็นเปอร์เซ็นต์ของขนาดทั้งหมด
                    fill="#ffffff" // สีขาวสำหรับข้อความ
                    stroke="#000000" // สีดำสำหรับขอบข้อความ
                    strokeWidth="1" // ความหนาของขอบข้อความ
                    transform={`rotate(-120 ${size / 2} ${size / 2})`} // หมุนข้อความที่จุดกลาง
                    style={{ 
                        pointerEvents: 'none', 
                        zIndex: 3, 
                        filter: 'drop-shadow(1px 1px 1px rgba(0, 0, 0, 0.7))', // เงาสีดำที่เบลอ
        ...
        <tspan
          ...
          fontSize={`${size * 0.15}px`} // ขนาดฟอนต์สำหรับ kW
        >
          {' Km'}
        </tspan>
        <tspan
